...
<body>
  ...
  <?php
    //Envia corre electrónicos a los usuarios seleccionados
    if (isset($_POST['Action']) and $_POST['Action']=='Enviar')
    {
      $emails = $_POST['emails'];
      ...
      $fecha_name = date("dmY", strtotime($fecha));
      $hora_fin = date("His", strtotime($hora_fin));
      $minuta_nombre = 'M' . $fecha_name . $hora_fin . '.pdf';

      //Close and output PDF document
      $pdfFile = $pdf->Output($minuta_nombre, 'S');
      ...
      try
      {
        ...
        $mail->SetFrom("minutasEnvia@gmail.com");
        $mail->Subject = "Minuta " . $fecha . $hora;
        $mail->Body = "Se adjunta el PDF del registro de la reunión realizada en el día " . $fecha . " a las " . $hora . " de la comisión/cuerpo colegido " . $cuerpo . ".";
        ...
        //Attachments
        $mail->AddStringAttachment($pdfFile, $minuta_nombre, 'base64', 'application/pdf');

        if(!$mail->send())
        {
          ?>
            <center><h5 class="indigo-text"><?php echo 'Correo no enviado. Error: ', $mail->ErrorInfo;?></h5></center>
          <?php
        }
        else
        {
          ob_end_clean();
          ?>
            <center><h5 class="indigo-text">Enviado con éxito.</h5></center>
          <?php
        }
      }
      catch (Exception $e)
      {
        ob_end_clean();
        ?>
          <center><h5 class="indigo-text"><?php echo 'No se pudo enviar. Error: ', $mail->ErrorInfo;?></h5></center>
        <?php
      }
    }
  ?>
  <main>
    <div id="rigth_section">
      <div class="col s12 m8 offset-m2 l6 offset-l3">
        <div id="left-section">
          <?php
            //Se hace la consulta para cambiar la contraseña
            if (isset($_POST['pass']))
            {
              $pass_old = $_POST["pass_old"];
              $write_pass_old = $_POST["password_old"];
              ...
              if($pass_old == $write_pass_old)
              {
                try
                {
                  $sql = "UPDATE docente SET password = '$pass', updated_at = '$fecha_hora' WHERE dc_codigo = '$id_user';";
                  $s = $pdo->prepare($sql);
                  $s->bindValue(':id', $id_user);
                  $s->execute();
                  ?>
                    <label class="#1a237e indigo-text darken-1">La contraseña se ha cambiado con éxito.</label>
                  <?php
                }
                catch(PDOException $e)
                {
              ...
              {
                ?>
                  <label class="#f44336 red-text">No se pudo cambiar la contraseña, la contraseña antigua enviada no coincide con la de la base de datos.</label>
                <?php
              }
            }
          ?>
    ...
    <!-- Modal para cambiar contraseña-->
      <div id="modalChangePass" class="modal">
        <div class="modal-content">
          <div id="left-section">
            <button class="waves-effect waves-light btn #1a237e indigo darken-1" name='close' id="closeChangeEmail">
